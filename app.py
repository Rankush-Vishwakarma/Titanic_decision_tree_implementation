...
def loadModel():
    try:
        with open(r"titanic_model.sav", "rb") as LR_input:
...
def predict(data):
    try:
        ...
        print(d)
        LR = loadModel()
        model['Decision Tree'] = LR.predict(d)
            ...
            if request.method == 'POST':
                ...
                def dict2htmltable(data):
                    try:
                        ...
                        if class_ == 0:
                            result  = 'You will not Survive sir!';
                        if class_ ==1:
                            result  = 'Congratulation! You have sucessfully survived.';
